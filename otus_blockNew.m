function [resI]=otus_blockNew(I,nbrBlock)
...
for k=1:nbrBlock
    ...
    [IDX1,T11,T21]=otus3level(upBlock(:,:,k));
    [IDX2,T12,T22]=otus3level(downBlock(:,:,k));
 ...
 %% otus operation on upBlocks   
     for j=1:c1
        ...
        upclass(j)=max(max(max(prob_11,prob_21),prob_31));
 ...
 %% otus operation on dowmBlocks
    for j=1:c2
        ...
        downclass(j)=max(max(max(prob_12,prob_22),prob_32));
        
        if prob_12==downclass(j)
            downBlock(:,j,k)=0;
        elseif prob_22==downclass(j)
            downBlock(:,j,k)=127;
        else prob_32==downclass(j)
            downBlock(:,j,k)=255;
        end
    end   
end
...
[IDX3,t11,t21]=otus3level(upLastBlock);
[r3,c3]=size(IDX3);
prob_13=0;
prob_23=0;
prob_33=0;
...
for j=1:c3
    ...
    upLastclass(j)=max(max(max(prob_13,prob_23),prob_33));
...
[IDX4,t12,t22]=otus3level(downLastBlock);
[r4,c4]=size(IDX4);
prob_14=0;
prob_24=0;
prob_34=0;
...
for j=1:c4
    ...
    downLastclass(j)=max(max(max(prob_14,prob_24),prob_34));
