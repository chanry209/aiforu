...
%     position=infos(2:5);
    ...
    %figure,colormap gray;imagesc(bbw);
    mbuvard=butterfilt(median(bbw),pixelpercm,0,10,4,1);
    %figure,colormap gray;imagesc(mbuvard);
    ...
    leftsig=...
        butterfilt(median(imbuvard(end-position(4):end-position(3),...
        mgauche-5:mgauche+5)'),pixelpercm,0,15,4,1);
    ...
    artleft=find(leftsig(2:end-1)>7 & ...
        leftsig(1:end-2)<leftsig(2:end-1) &...
        leftsig(3:end)<=leftsig(2:end-1) )+1;
    
    imbands=[imbuvard(end-position(4):end-position(3),mgauche-5:mgauche+5) ...
        zeros(position(4)-position(3)+1,1) ...
        imbuvard(end-position(4):end-position(3),mdroite-5:mdroite+5)];
    %figure,colormap gray;imagesc(imbands);
