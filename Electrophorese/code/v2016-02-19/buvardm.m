classdef buvardm < hgsetget
    ...
    methods
        function obj=buvardm(im)
            ...
            obj.Fig=figure('MenuBar','None','WindowButtonMotionFcn',@(src,evt) MouseMovement(obj),'WindowButtonDownFcn',@(src,evt) MouseDown(obj),'WindowButtonUpFcn',@(src,evt) MouseUp(obj),'Name',obj.filename);
        ...
        function MouseUp(obj)
            ...
            elseif(obj.getMode()==2)
                ...
                filename=[obj.filename(1:end-4) sprintf('_A%d_%d-%dx%d-%d.png',round(1800/pi*obj.Angle),r(1:2,1:2))];
                if(isempty(obj.profilobj) || ~isvalid(obj.profilobj))
                    obj.profilobj=profilmanual(imProfil,obj.pixelpercm,filename);
                else
                    obj.profilobj.setImage(imProfil,obj.pixelpercm);
                    obj.profilobj.setfilename(filename);
                end
