function modelGMMElectrophorese
% modified by WANG Z. (FMM - Institut Catholique Lille)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nbStates = input('How many gaussian models you want to try? \n '); % nombre de models

%% Load a dataset consisting of 3 demonstrations of a 2D signal.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%load('data/data1.mat'); %load 'Data'
[f,p]=uigetfile('*.mat;*.mat');
filename=[p f];
[temp,name,ext] = fileparts(f);

load(filename);
x= (1:length(scorec))/length(scorec);
Data = [x; scorec'];
Data = [1:length(scorec); Data];
%Data = scorec';
...
% Data = [x;Data];
% figure
% plot(Data(1,:), Data(2,:), 'x', 'markerSize', 4, 'color', [.3 .3 .3]);
% axis([min(Data(1,:)) max(Data(1,:)) min(Data(2,:))-0.01 max(Data(2,:))+0.01]);
% xlabel('Pixel','fontsize',16); ylabel('Intentity','fontsize',16);
% hold on 
% y1 = gaussmf(x,[Sigma(1,1,1) Mu(1)]);
% plot(x,y1)
% 
% y2=(1/((sqrt(2*pi))*sigma))*exp(-((x-a).^2)/(2*sigma.^2));
% 
% dataMin = min(Data);
% dataMax = max(Data);
...
%% Plot of the data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('position',[10,10,1000,800],'name',strcat('GMM-GMR-',name));
%plot 1D
for n=1:nbVar-1
...
text(0.3,0.9,'\bf Profle of data','fontsize',12)
...
hold on
plot(Data(2,:), Data(3,:), 'lineWidth', 2, 'color', [.3 .3 .3]);
text(0.3,0.9,'\bf Profle of GMMs ','fontsize',12)
axis([min(Data(2,:))-0.01 max(Data(2,:))+0.01 min(Data(3,:))-0.01 max(Data(3,:))+0.01]);
xlabel('x_1','fontsize',16); ylabel('x_2','fontsize',16);
...
text(0.15,0.9,' \bf Regression Profile of GMMs','fontsize',12)
axis([min(Data(2,:))-0.01 max(Data(2,:))+0.01 min(Data(3,:))-0.01 max(Data(3,:))+0.01]);
xlabel('x_1','fontsize',16); ylabel('x_2','fontsize',16);
