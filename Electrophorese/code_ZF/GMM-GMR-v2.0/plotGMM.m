...
if display_mode==1
  ...
  for j=1:nbData
    ...
    %%%%%%% plot Gaussian model
    xGaussian = 0:0.01:1;
    %sigmaGaussian = sqrt(real(Sigma(2,2,j))); %compter que la variance en 2,2
    sigmaGaussian = sqrt( trace(real(Sigma(:,:,j)))); %compter tous les variances
    y1 = Mu(2,j)*gaussmf(xGaussian,[sigmaGaussian Mu(1,j)]);
    plot(xGaussian ,y1, 'lineWidth', 2)
    % y2=Mu(2,j)*(1./((sqrt(2*pi))*sigmaGaussian(1,1)))*exp(-((xGaussian-Mu(1,j)).^2)/(2*sigmaGaussian(1,1).^2));
    %plot(xGaussian ,y2)
    %%%%%%%%
...
elseif display_mode==2
  ...
  for j=1:nbData
    ...
    %%%%%%% plot Gaussian model
    xGaussian = 0:0.01:1;
    sigmaGaussian = sqrt(real(Sigma(2,2,j)));
    %sigmaGaussian = sqrt( trace(real(Sigma(:,:,j))));
    y1 = Mu(2,j)*gaussmf(xGaussian,[sigmaGaussian Mu(1,j)]);
    plot(xGaussian ,y1, 'lineWidth', 0.5)
    % y2=Mu(2,j)*(1./((sqrt(2*pi))*sigmaGaussian(1,1)))*exp(-((xGaussian-Mu(1,j)).^2)/(2*sigmaGaussian(1,1).^2));
    %plot(xGaussian ,y2)
    %%%%%%%%
