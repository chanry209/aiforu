...
Labeltest1=ones(length(data_test1),1);

indices = ones(length(num_panne),1);
[train,test] = crossvalind('holdout',indices,0.2);
traindata2 = num_panne(train,:);
data_test2 = num_panne(test,:);
Labeltest2=ones(length(data_test2),1)*2;
...
[mtrs1,class1] = GaussianMixture(traindata1, Kinit, Kfinal, false);
...
[mtrs2,class2] = GaussianMixture(traindata2, Kinit, Kfinal, false);
...
Taux_Rec(i) = numel(find(Labeltest==class))/length(Labeltest)*100;
