...
function model = NetAAT_momentum(input,output,n,err_glob,option,lr,lamda,affichage)
...
clear W1 B1 W2 B2 Ni Nc Base data_train data_test Base BaseCrv    = data_test;
    ...
    if it>1
     ...
     fprintf('Iteration:%d  ; Error Train : %f  ; TL : %f  \n',it,L(it),TL)
        ...
        if ((L(it) - TL*err_ratio)>0.00001)&&(lr>=0.00001)% if lr ==0, the study will be stop
                    ...
                    if affichage==1
            fprintf('Case1 :  ((L(it) - TL*err_ratio)>0.00001)&&(lr>=0.00001) \n')
                    end
...
hold off
    ...
    hold on
    plot(1:n,L,'r-.')
    title('Result of the crossvalidation_NetAT')
    xlabel('Iteration')
    ylabel('Quadratic Error')
    plot(bestit,min_valid,'bo','markersize',16)
    legend('CV - 20%','Train - 80%')    
